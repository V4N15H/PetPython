...
API_URL = "https://api-seller.ozon.ru/v2/posting/fbo/list"
OUTPUT_FILE = "ozon_clusters.xlsx"


# ============ 2. Функции ============
def get_posting_numbers():
    """
    Парсит сайт заказов и извлекает номера отправлений (posting_number),
    где время доставки = 28 часов.
    """
    ...
    while True:
        ...
        "filter": {
            "company_id": "290929",
            "processed_at_from": (datetime.now(timezone.utc) - timedelta(days=3))
            .isoformat(timespec="seconds")
            .replace("+00:00", "Z"),
            "processed_at_to": datetime.now(timezone.utc)
            .isoformat(timespec="seconds")
            .replace("+00:00", "Z"),
        },
        "limit": "1000",
        ...
        try:
            response = client.post(url=SITE_API_URL, headers=headers, json=json_payload, follow_redirects=True)
            data = response.json()
            result = data.get("result")
            cursor = result.get("cursor")
            postings = result.get("postings")
            for p in postings:
                est_time = p.get("estimated_delivery_time_hours")
                # Фильтр по времени доставки (точное совпадение)
                if est_time != "28":
                        continue
            ...
            print("Время ожидания вышло, прекращаю цикл")
            break
    return posting_numbers


def get_api_data(posting_numbers):
    """
    Получает данные о городe, складe и связанных кластерах через API.
    """
    headers = {
    ...
    while True:
        ...
        if not result or result == []:
            print("Достигнут предел пагинации")
            break
        for item in result:
            posting_number = item.get("posting_number")
            if posting_number in posting_numbers: #and posting_number not in seen_postings:
                seen_postings.append(posting_number)
                analytics = item.get("analytics_data", {})
                finances = item.get("financial_data", {})
                results.append(
                    {
        ...
        counter += 1
        json_payload["offset"] = 1000 * counter
    return results
...
def main():
    ...
    posting_numbers = get_posting_numbers()
    print("Получение данных из API...")
    results = get_api_data(posting_numbers)
